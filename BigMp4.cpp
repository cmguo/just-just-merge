...
namespace ppbox
{
    namespace segment
    {
        ...
        BigMp4::BigMp4(
            ...
            :io_svc_(io_svc)
            ,vod_(util::daemon::use_module<ppbox::peer::PeerModule>(global_daemon()))
            ,http_client_(io_svc)
            ,recv_size_(0)
            ,down_load_size_(0)
            ,head_size_(0)
        ...
        {
            LOG_ERROR("[async_open] mode:"<<mode
                <<" playlink:"<<playlink);
        ...
        void BigMp4::async_tranfer(
            ...
            ,response_type const &resp)
        {
            LOG_ERROR("[async_tranfer] iSize:"<<iSize);
        ...
        void BigMp4::close()
        {
            ...
            LOG_ERROR("[close] StepType:"<<open_step_);
        ...
        {
            LOG_ERROR("[handle_async_open] StepType:"<<open_step_
                <<" ec:"<<ec.message());
            ...
            std::ostream* stream)
        {
            ...
            LOG_DEBUG("[async_tranfer_bighead] Range from:"<<0
        ...
        void BigMp4::open_bighead_callback(boost::system::error_code const & ec)
        {
            ...
            if(ec)
            {
                LOG_ERROR("[open_bighead_callback] ec:"<<ec.message());
                handle_async_open(ec);
            }
            else 
            {
        ...
        void BigMp4::down_async_open(boost::system::error_code const & ec)
        {
            ...
            if(ec)
            {
                LOG_ERROR("[down_async_open] ec:"<<ec.message());
                handle_async_open(ec);
            }
            else 
            {
            ...
            ,std::size_t bytes_transferred)
        {
            if(ec)
            {
                LOG_ERROR("[download_handler] ec:"<<ec.message());
                handle_async_open(ec);
            ...
            else
            {
                if(bytes_transferred > 0)
                {
                    ...
                    {
                        LOG_ERROR("[download_handler] Finish");
                        if(ec1)
                        {
                            handle_async_open(ec1);
                            return;
                ...
                {
                    LOG_ERROR("[download_handler] download size < 1");
                    handle_async_open(ec);
                }
            }
        }
            ...
            boost::system::error_code & ec)
        {
            ...
            LOG_DEBUG("[get_cdn_request] Range from:"<<recv_size_
                <<" to:"<<down_load_size_
            ...
            boost::system::error_code & ec)
        {
            ...
            LOG_DEBUG("[get_request] Range from:"<<recv_size_
                <<" to:"<<down_load_size_
                <<" cdn url:"<<real_url.to_string());

            LOG_DEBUG("[get_request] full url:"<<url.to_string());
            return ec;
        }
